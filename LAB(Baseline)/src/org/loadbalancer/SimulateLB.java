...
public class SimulateLB {
    static ArrayList<IoTDevice> generateIoTDevices() {
        ArrayList<IoTDevice> list = new ArrayList<>();
        int gap = 25;
        int max = 8;
        for (int i = 0; i < max; i++) {
            for (int j = 0; j < max; j++) {
                list.add(new IoTDevice(new Coordinates(i*gap, j*gap)));
            }
        }
        return list;
    }
    ...
    static ArrayList<FogNode> fogNodes = new ArrayList<FogNode>(){{
        add(new FogNode(new Coordinates(12.5, 12.5)));
        add(new FogNode(new Coordinates(12.5, 62.5)));
        add(new FogNode(new Coordinates(62.5, 62.5)));
        add(new FogNode(new Coordinates(112.5, 12.5)));
        add(new FogNode(new Coordinates(12.5, 112.5)));
        add(new FogNode(new Coordinates(62.5, 112.5)));
        add(new FogNode(new Coordinates(112.5, 62.5)));
        add(new FogNode(new Coordinates(162.5, 112.5)));
        add(new FogNode(new Coordinates(62.5, 162.5)));
    }};
    static ArrayList<IoTDevice> iotDevices = generateIoTDevices();
    static LoadBalancer lb = new LoadBalancer(fogNodes, iotDevices);
    ...
    private static void createFogDevices(int userId, String appId) {
        ...
        fogDevices.add(proxy); /* TODO: keep proxy to group clusters into areas */
//        for(int i=0;i<numOfAreas;i++){
//            /* TODO(now): modify into add multiple areas and construct topology for each area */
//            addArea(i+"", userId, appId, proxy.getId());
//        }
        for (int i = 0; i < lb.fogNodes.size(); i++) {
            addArea(i, userId, appId, proxy.getId());
        }
    }

    private static FogDevice addArea(int id, int userId, String appId, int parentId){
        FogDevice router = createFogDevice("d-"+id, 2800, 4000, 10000, 10000, 1, 0.0, 107.339, 83.4333);
        fogDevices.add(router);
        router.setUplinkLatency(2); // latency of connection between router and proxy server is 2 ms
        /* TODO: association algorithm result goes here */
        for(int i=0;i<lb.iotDevices.size();i++){
            if (lb.association.get(id).get(i)) {
